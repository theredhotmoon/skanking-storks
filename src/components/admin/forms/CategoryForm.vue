...
const emit = defineEmits<{ submit: [CategoryPayload]; cancel: [] }>()

const form = reactive<CategoryPayload>({ name: '' })
...
<template>
  <form @submit.prevent="submit" class="flex flex-col gap-4">
    <div>
      ...
      <input v-model="form.name" required class="field-input" placeholder="Category name" />
